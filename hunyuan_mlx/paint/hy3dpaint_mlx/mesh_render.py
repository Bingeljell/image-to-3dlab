...
class MeshRender:
    ...
    # --- image-to-3dlab: i2l_occlusion_fill -----------------------------------------------------
    @staticmethod
    def inpaint_occlusion_aware(texture, tex_pos, chart_mask, covered_mask, pad_px=8):
        """i2l_occlusion_fill: fill texels the way `inpaint` does, except a texel a camera never saw
        (a concave crease self-occluded from every fixed view angle) is filled from its
        nearest neighbor in 3D SURFACE space, not 2D atlas-pixel space. Plain EDT is
        atlas-pixel-nearest, which can grab an unrelated chart's color purely because
        xatlas happened to pack it nearby -- the mechanism behind the shattered texture
        tear on concave geometry.

        tex_pos/chart_mask come from uv_rasterize; covered_mask from bake/bake_multi.
        Texels outside any UV chart (normal atlas packing gaps) are irrelevant to real
        rendering and only get a small bounded pad for mip-map safety, not a full fill.
        """
        from scipy import ndimage
        ...
        occluded_in_chart = chart_mask & ~covered_mask
        if occluded_in_chart.any() and covered_mask.any():
            visible_idx = np.argwhere(covered_mask)
            occ_idx = np.argwhere(occluded_in_chart)
            tree = cKDTree(tex_pos[covered_mask])
            _, nn = tree.query(tex_pos[occluded_in_chart], k=1)
            out[occ_idx[:, 0], occ_idx[:, 1]] = out[visible_idx[nn][:, 0], visible_idx[nn][:, 1]]

        background = ~chart_mask
        if background.any() and chart_mask.any():
            dist, idx = ndimage.distance_transform_edt(
                background, return_distances=True, return_indices=True)
            filled = out[tuple(idx)]
            within = dist <= pad_px
            out[within & background] = filled[within & background]
            out[~within & background] = 0.5
        return np.clip(out, 0, 1)

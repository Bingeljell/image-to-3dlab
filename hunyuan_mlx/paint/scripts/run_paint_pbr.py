...
def main():
    ...
    mx.random.seed(int(os.environ.get("PAINT_SEED", "0")))
    ...
    # i2l_occlusion_fill: occluded texels (concave creases no camera sees) filled from their nearest
    # neighbor on the real 3D surface, not nearest in 2D atlas-pixel space -- see mesh_render.py
    tex_pos, _, chart_mask = rend.uv_rasterize(TEX)
    tex_a = rend.inpaint_occlusion_aware(tex_a, tex_pos, chart_mask, cov)
    tex_m = rend.inpaint_occlusion_aware(tex_m, tex_pos, chart_mask, cov)
